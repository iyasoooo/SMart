...
$chat_intro	= (isset($_REQUEST['chat_intro'])) ? trim($_REQUEST['chat_intro']) : '';
$chat		= (isset($_REQUEST['chat'])) ? trim($_REQUEST['chat']) : '';
...
{	
	...
	INSERT INTO `chat`(`id_chat`, `id_customer`, `id_seller`, `chat`, `created_by`, `created_date`) 
			VALUES (NULL,'$id_customer','$id_seller','$chat', '$id_customer', NOW())";		
	...
	print "<script>self.location='sembang.php?id_customer=".$id_customer."&id_seller=".$id_seller."&chat=';</script>";
}
...
if($act == "del")
{
	...
	print "<script>self.location='sembang.php?id_customer=".$id_customer."&id_seller=".$id_seller."';</script>";
}
?>
<!DOCTYPE html>
<html>
...
<div class="" >
	...
	<div class="w3-containerx w3-paddingx" id="contact">
		<div class="w3-contentx w3-container" style="max-width:600px">
			...
			<?PHP
			$bil = 0;
			$SQL_list = "SELECT * FROM `chat` WHERE `id_customer` = $id_customer AND `id_seller` = $id_seller";
	...
	<div class="w3-card w3-white w3-paddingx w3-padding-small w3-round">	
		...
		<form action="" method="post">
			<div class="w3-padding">
				<textarea class="w3-input w3-border w3-round" rows="2" name="chat" required><?PHP echo $chat_intro;?></textarea>
				<div class="w3-padding-small"></div>
